...
function query(filterBy) {
  if (filterBy) {
    ...
    const filteredBooks = gBooks.filter(book => {
      return book.language.includes(lang) && book.title.includes(title) && book.publishedDate === year
    })
...
function addReview(bookId, review) {
  var bookPos = gBooks.findIndex(book => book.id === bookId)
  var book = gBooks[bookPos]
  book['Reviews'] = [review]
